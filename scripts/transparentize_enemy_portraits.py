...
SRC_DIR = Path(r"E:\AIGame\resource\enemy")
DST_DIR = ROOT / "assets" / "media" / "enemys"
...
PORTRAIT_UPDATES = {
    "enemy_018": "assets/media/enemys/langA.png",
    "enemy_019": "assets/media/enemys/langB.png",
    "enemy_020": "assets/media/enemys/langC.png",
}
...
def update_catalog_portraits() -> None:
    ...
    for enemy_id, portrait in PORTRAIT_UPDATES.items():
        marker = f'"id": "{enemy_id}"'
        enemy_index = text.find(marker)
        if enemy_index == -1:
            continue
        portrait_key = '"portrait": "'
        portrait_index = text.find(portrait_key, enemy_index)
        if portrait_index == -1:
            continue
